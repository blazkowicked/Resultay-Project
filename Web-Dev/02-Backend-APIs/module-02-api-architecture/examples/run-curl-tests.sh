...
#      (e.g. {"status":"success", ...} or an error code like NOT_FOUND).
...
# -----------------------------------------------------------------------------
#  check <name> <expected_code> <full_output>
#         <marker> [<marker> ...]
...
printf '%s\n' "== API contract checks (spec-template/api-spec.md) =="
echo

# --- 1) GET /projects .................................. expect 200
out="$(get_projects)"
check "GET /projects" 200 "$out" '"status": "success"' '"pagination"'

# --- 2) GET /projects?page=1&limit=1 ................... expect 200
out="$(get_projects_paged 1 1)"
check "GET /projects (paged)" 200 "$out" '"status": "success"' '"limit": 1' '"total"'

# --- 3) POST /projects (valid) ......................... expect 201
out="$(create_project)"
check "POST /projects" 201 "$out" '"status": "success"' '"id"' '"created_at"'
PROJECT_ID="$(extract_id "$out")"

# --- 4) POST /projects (invalid body) .................. expect 400
out="$(create_project '{"description":"missing the required title field"}')"
check "POST /projects (invalid body)" 400 "$out" '"status": "error"' '"code": "VALIDATION_ERROR"'

# --- 5) GET /projects/:id (nonexistent id) ............. expect 404
out="$(get_project '00000000-0000-0000-0000-000000000000')"
check "GET /projects/:id (missing)" 404 "$out" '"status": "error"' '"code": "NOT_FOUND"'
...
if [[ -z "${PROJECT_ID:-}" ]]; then
  ...
  check "GET /projects/:id" 200 "$out" '"status": "success"' '"id"'

  # --- 7) PATCH /projects/:id .......................... expect 200
  out="$(patch_project "$ProjectId" '{"description":"Updated via PATCH"}')"
  check "PATCH /projects/:id" 200 "$out" '"status": "success"' '"description": "Updated via PATCH"'

  # --- 8) DELETE /projects/:id ......................... expect 204
  out="$(delete_project "$ProjectId")"
  check "DELETE /projects/:id" 204 "$out"

  # --- 9) GET /projects/:id after delete ............... expect 404
  out="$(get_project "$ProjectId")"
  check "GET /projects/:id after delete" 404 "$out" '"status": "error"' '"code": "NOT_FOUND"'
fi
